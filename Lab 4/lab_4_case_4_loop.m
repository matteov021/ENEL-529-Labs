...
var_list = [0.01, 0.25, 1.0, 5.0];

for j = 1:length(var_list)

    agwn_var = var_list(j);
    std_agwn = sqrt(agwn_var);
    ...
    for i = 1:1000
        for n = 1:N
            S_i(end+1) = b(i) * sqrt(2*E/T) * cos(2*pi*fc*delta_t*(N*(i-1)+n)+unifrnd(0,2*pi));
        end
    end
    ...
    for i = 1:1000*N
        r(end+1) = raylrnd(1) * S_i(i) + normrnd(0, std_agwn);
    end
    ...
    for n = 1:1000*N
        x(end+1) = r_n(n) * (2*cos(2*pi*fc*delta_t*n));
    end
